...
contract("Springboard", accounts => {
   ...
   it("Upgrade wallet v1 to v2 should work", async () => {
      ...
      console.log(`wallet Address: ${walletAddress},`, `Current Version: ${version}`);
      assert.equal(version, "1.0", "version should be 1.0");
         
      // Write you code here....
      // 1) Upgrade wallet to V2
      await walletV1.die();
      let runtimeCode_v2 = WalletV2.deployedBytecode;
      tx = await springboard.execute(runtimeCode_v2);
      ...
      console.log(`wallet Address: ${walletAddress},`,`New version: ${version}`);
